...
try:
    # Load the SpEffectTemplate.csv file from the 'template' folder
    script_dir = os.path.dirname(os.path.abspath(__file__))
    template_file = os.path.join(script_dir, 'template', 'SpEffectTemplate.csv')
    ...
    if 'Unnamed: 372' in new_df.columns:
        new_df.drop(columns=['Unnamed: 372'], inplace=True)

    # Rename the 'spiritDeathSpEffectId' column to 'spiritDeathSpEffectId,'
    if 'spiritDeathSpEffectId' in new_df.columns:
        new_df.rename(columns={'spiritDeathSpEffectId': 'spiritDeathSpEffectId,'}, inplace=True)
