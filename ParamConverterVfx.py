...
try:
    # Load the SpEffectVfxParamTemplate.csv file from the 'template' folder
    script_dir = os.path.dirname(os.path.abspath(__file__))
    template_file = os.path.join(script_dir, 'template', 'SpEffectVfxParamTemplate.csv')
    ...
    # Set the 'pad' column to '[0|0|0|0|0|0|0|0|0]'
    if 'pad' in new_df.columns:
        new_df['pad'] = '[0|0|0|0|0|0|0|0|0]'

    # Ensure no extra columns are added
    if 'Unnamed: 72' in new_df.columns:
        new_df.drop(columns=['Unnamed: 72'], inplace=True)

    # Rename the 'pad' column to 'pad,'
    if 'pad' in new_df.columns:
        new_df.rename(columns={'pad': 'pad,'}, inplace=True)
